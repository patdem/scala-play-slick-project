...
class OAuth2InfoRepository @Inject()(dbConfigProvider: DatabaseConfigProvider)
                                    (implicit ec: ExecutionContext, implicit val classTag: ClassTag[OAuth2Info])
  ...
  case class OAuth2InfoDto(id: Long, providerId: String, providerKey: String, accessToken: String,
                           tokenType: Option[String], expiresIn: Option[Int])
  ...
  class OAuth2InfoTable(tag: Tag) extends Table[OAuth2InfoDto](tag, "oAuth2Info") {
    ...
    def * = (id, providerId, providerKey, accessToken, tokenType, expiresIn) <> ((OAuth2InfoDto.apply _).tupled,
      OAuth2InfoDto.unapply)
  }
  ...
  override def add(loginInfo: LoginInfo, authInfo: OAuth2Info): Future[OAuth2Info] = db.run {
    (oAuth2Info.map(c => (c.providerId, c.providerKey, c.accessToken, c.tokenType, c.expiresIn))
      ...
      into { case ((providerId, providerKey, accessToken, tokenType, expiresIn), id) =>
